...
static inline void tree_lock(tctree_t * t)
{
    if (t->locking)
	pthread_mutex_lock(&t->lock);
}

static inline void tree_unlock(tctree_t * t)
{
    if (t->locking)
	pthread_mutex_unlock(&t->lock);
}

extern tctree_t *tctree_new(int lock, tccompare_fn cmp, uint32_t flags)
{
...
static void free_nodes(dnode_t * node, dnode_t * nil, tcfree_fn fr)
{
    if (node == nil)
	return;
...
static dnode_t *do_find(tctree_t * dict, void *key)
{
...
extern int tctree_find(tctree_t * dict, void *key, void *r)
{
    dnode_t *node;
    void **ret = r;
...
static int do_search(tctree_t * dict, void *key, void *r, int replace)
{
...
extern int tctree_delete(tctree_t * dict, void *key, void *r)
{
