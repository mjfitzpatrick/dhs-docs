...
void dhsUtilSysOpen (
  ...
  if ((*dhsID = dcaConnect (dhsSup.node, dhsSup.port)) < 0)
     set error and return

  format SYS_OPEN message e.g. "SYS_OPEN:NOCS" or "SYS_OPEN:MSL"
  if (dcaSendMsg (dhsID, message) == DHS_ERROR)
     set error and return

  get any control settings from the supervisor
