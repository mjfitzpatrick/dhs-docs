 ...
 * __doc__  \item[\sc use:] \emph{void dhsUtilSysOpen(long $\star$istat, char $\star$resp, 
 * __doc__    dhsHandle $\star$dhsID, ulong whoami);}
 ...
 * dhsUtilSysOpen ( ... )
 *******************************************************************************/ 
void dhsUtilSysOpen ( long *istat,      /* inherited status                    */
    ...
    ) 
{
	...
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsSysOpen:  bad parameter, dhsHandle ptr == NULL.");
	return;
    }
    ...
    {
	*istat = DHS_ERROR;
	sprintf(resp,"dhsSysOpe:  bad value, %s.", 
			((*dhsID<(dhsHandle)0) ? "dhsHandle invalid" : "whoami value invalid"));
    ...
    *istat = DHS_OK;
    MNSN_RET_SUCCESS(resp,"dhsSysOpen:  Success");
    /* return */ 
    return;
}
