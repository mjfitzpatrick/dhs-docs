 ...
 * __doc__  \item[\sc use:] \emph{void dhsUtilOpenConnect(long $\star$istat, char $\star$resp, 
 * __doc__    dhsHandle $\star$dhsID, ulong whoami, fpConfig\_t $\star$cfg);}
 ...
 * dhsUtilOpenConnect ( ... )
 *******************************************************************************/ 
void dhsUtilOpenConnect ( long *istat,      /* inherited status                */
    ...
    ) 
{
    ...
    {
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsOpenConnect:  bad parameter dhsHandle pointer == NULL");
	return;
    }
    ...
    {
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsOpenConnect: Failed bad whoami value");
	return;
    }
