    ...
    ) 
{
	...
	*istat = DHS_ERROR;
	sprintf(resp,"dhsSendMetaData: bad parameter, %s",
			((cfg == (mdConfig_t *)NULL))?"MetaData configuration==NULL":"expID==NULL");
    ...
    {
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsSendMetaData: Bad parameter. dhs not open or too many fields. ");
	return;
    }
    ...
    {
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsSendMetaData: bad parameters BlkAddr==NULL and size!=0");
	return;
    }
    ...
    {
	*istat = DHS_OK;
	MNSN_RET_FAILURE(resp,"dhsSendMetaData: Success. blkSize==0");
	return;
    }
	...
	{
	    *istat = DHS_ERROR;
	    sprintf(resp,"dhsSendMetaData: fits container file create failed. fitsStatus=%d",fitsStatus);
	    MNSN_RET_FAILURE(resp,"");
	    return;
	}
	...
	{
	    *istat = DHS_ERROR;
	    sprintf(resp,"dhsSendMetaData: fits container file  open failed. fitsStatus=%d",fitsStatus);
	    MNSN_RET_FAILURE(resp,"");
	    return;
	}
    }
    ...
    switch (cfg->metaType) 
    {
	  ...
	  for ( fc=0; fc<nr; fc++ ) 
	  {
	      ...
	      {
		  *istat = DHS_OK;
		  sprintf(resp,"dhsSendMetaData: fits write failed at record %d with fitsStatus = %d", fc+1, fitsStatus);
		  MNSN_RET_FAILURE(resp,"");
	      }
	  }
	  break;
	  ...
	  {
	      *istat = DHS_ERROR;
	      sprintf(resp,"dhsSendMetaData:  fits ascii table create failed.  fitsStatus = %d", fitsStatus);
	      MNSN_RET_FAILURE(resp,"");
	      break;
	  }
	  ...
	  for ( fc=0; fc<nr; fc++ ) 
	  {
	      ...
	      {
		  *istat = DHS_ERROR;
		  sprintf(resp,"dhsSendMetaData:  fits ascii table write1 failed. fitsStatus = %d", fitsStatus);
		  MNSN_RET_FAILURE(resp,"");
		  break;
	      }
	      ...
	      {
		  *istat = DHS_ERROR;
		  sprintf(resp,"dhsSendMetaData:  fits ascii table write2 failed. fitsStatus = %d", fitsStatus);
		  MNSN_RET_FAILURE(resp,"");
		  break;
	      }
	      ...
	      {
		  sprintf(resp,"dhsSendMetaData:  fits ascii table write3 failed. fitsStatus = %d", fitsStatus);
		  MNSN_RET_FAILURE(resp,"");
		  break;
	      }
	  }
	  ...
	  *istat = DHS_OK;
	  MNSN_RET_FAILURE(resp,"dhsSendMetaData:  metadata type SHIFTLIST not yet implemented");
	  break;

      case DHS_MDTYPE_ARRAYDATA: /* MONSOON data array ??? */
	  *istat = DHS_OK;
	  MNSN_RET_FAILURE(resp,"dhsSendMetaData:  metadata type ARRAYDATA not yet implemented");
	  break;
    ...
    {
	*istat = DHS_ERROR;
	sprintf(resp,"dhsSendMetaData:  fits close failed. fitsStatus = %d", fitsStatus);
	MNSN_RET_FAILURE(resp,"");
	return;
    }
