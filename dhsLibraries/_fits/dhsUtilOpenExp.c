 ...
 * __doc__  \item[\sc use:] \emph{void dhsUtilOpenExp(long $\star$istat, 
 * __doc__    char $\star$resp, dhsHandle dhsID, fpConfig\_t $\star$cfg, double $\star$expID, char $\star$obsetID);}
 ...
 * dhsUtilOpenExp ( ... )
 *******************************************************************************/ 
void dhsUtilOpenExp ( long *istat,     /* inherited status                     */
    ...
    ) 
{
	...
	*istat = DHS_ERROR;
	sprintf(resp,"dhsOpenExp:  bad parameter, %s ",
			((cfg==(fpConfig_t*)NULL) ? "FP cfg pointer==NULL":
    ...
    {
	*istat = DHS_ERROR;
	MNSN_RET_FAILURE(resp,"dhsOpenExp: Bad parameter, dhsHandle invalid.");
	return;
    }
