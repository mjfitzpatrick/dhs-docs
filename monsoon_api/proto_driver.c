...
main()
{
   ...
   dhsSendPixelData (&status, response, dhs_id, buf, blkSize, &pan1,
		      &expID, obsSetID);
    ...
    * be the same as the pixel - name collision. SOLUTION: dhsSendMetaData
    * adds "meta" to the end of the filename. */

   bzero (buf, blkSize);
   strcpy (buf, "KEYWORD=VALUE\n");

   dhsSendMetaData (&status, response, dhs_id, buf, blkSize, &meta,
                     &expID, obsSetID);
   printf ("status = %ld response = %s", status, response);
