...
void
dhsSendMetaData (long *status, char *response, dhsHandle dhs_id,
                  ...
                  double *exposure_id, char *obsSet_id)
{
   ...
   if (fd == -1) {
      ...
      *status = -1;
      strcpy (response, "dhsSendMetaData() fails to open file\n");
      return;
   }
   ...
   if (write_cnt < 0) {
      ...
      *status = -1;
      strcpy (response, "dhsSendMetaData() fails to write file");
      close (fd);
      return;
   }
   write_cnt=0;
...
printf ("finished write of %d bytes of metadata in send_meta.c\n", write_cnt);
   ...
   /* send success */
   *status = 1;
   strcpy (response, "dhsSendMetaData succeeds\n");
